...
const onDragDropEvent = vi.hoisted(() =>
  vi.fn((_handler: (event: unknown) => void) => Promise.resolve(() => {})),
);
...
describe("NoteEditor", () => {
  ...
  describe("attachments (spec §11)", () => {
    function makeImageFile(name = "photo.png"): File {
      return new File([new Uint8Array([1, 2, 3])], name, { type: "image/png" });
    }

    function pasteInto(editable: HTMLElement, clipboardData: Partial<DataTransfer>) {
    ...
    it("claims a pasted image file, writes it, and inserts the returned reference at the caret", async () => {
      mockInvoke({ write_attachment: "attachment:01NEW" });

      render(<ControlledNoteEditor rootId="01ROOT" path="note.md" />);
      const editable = await screen.findByRole("textbox");

      const imageFile = makeImageFile();
      const handled = pasteInto(editable, { files: [imageFile] as unknown as FileList, getData: () => "" });
      ...
      await screen.findByText(/!\[image\]\(attachment:01NEW\)/);
    });

    it("shows an inline error when a pasted image fails to write", async () => {
      invoke.mockImplementation((command: string) => {
        if (command === "open_note") return Promise.resolve({ content: "Loaded\n", id: "01LOADED", is_conflicted: false });
        if (command === "write_attachment") return Promise.reject(new Error("not a recognized image format"));
        return Promise.resolve(undefined);
      });
      ...
      pasteInto(editable, { files: [makeImageFile()] as unknown as FileList, getData: () => "" });
    ...
    it("claims a pasted file:// URI, imports it by path, and inserts the reference", async () => {
      mockInvoke({ import_attachment: "attachment:01URI" });

      render(<ControlledNoteEditor rootId="01ROOT" path="note.md" />);
      const editable = await screen.findByRole("textbox");

      const handled = pasteInto(editable, {
        files: [] as unknown as FileList,
        getData: (format: string) => (format === "text/uri-list" ? "file:///home/user/pic.png" : ""),
      });
      ...
      await waitFor(() =>
        expect(invoke).toHaveBeenCalledWith("import_attachment", {
          rootId: "01ROOT",
          absolutePath: "/home/user/pic.png",
        }),
      );
      await screen.findByText(/!\[image\]\(attachment:01URI\)/);
    });

    it("does not claim an ordinary text paste, leaving it to CodeMirror's default handling", async () => {
      render(<ControlledNoteEditor rootId="01ROOT" path="note.md" />);
      const editable = await screen.findByRole("textbox");

      pasteInto(editable, { files: [] as unknown as FileList, getData: () => "" });
    ...
    it("attaches an image file chosen via the toolbar's file picker at the caret", async () => {
      const user = userEvent.setup();
      mockInvoke({ pick_image_file: "/home/user/chosen.png", import_attachment: "attachment:01PICKED" });
      ...
      await waitFor(() =>
        expect(invoke).toHaveBeenCalledWith("import_attachment", {
          rootId: "01ROOT",
          absolutePath: "/home/user/chosen.png",
        }),
      );
      await screen.findByText(/!\[image\]\(attachment:01PICKED\)/);
    });

    it("shows an inline error when the file picker itself fails", async () => {
      const user = userEvent.setup();
      invoke.mockImplementation((command: string) => {
        if (command === "pick_image_file") return Promise.reject(new Error("dialog plugin unavailable"));
        if (command === "open_note") return Promise.resolve({ content: "Loaded\n", id: "01LOADED", is_conflicted: false });
        return Promise.resolve(undefined);
      });
      ...
      await screen.findByText("dialog plugin unavailable");
      expect(invoke).not.toHaveBeenCalledWith("import_attachment", expect.anything());
    });

    it("inserts nothing when the file picker is cancelled", async () => {
      const user = userEvent.setup();
    ...
    describe("native drag-and-drop", () => {
      function registeredDropHandler() {
        const [[handler]] = onDragDropEvent.mock.calls.slice(-1);
      ...
      function mockHostRect(container: HTMLElement) {
        const host = container.querySelector(".note-editor__cm-host") as HTMLElement;
      ...
      it("shows the drag-over affordance while a drag is over the editor host", async () => {
        const { container } = render(<ControlledNoteEditor rootId="01ROOT" path="note.md" />);
        await screen.findByText("Loaded");
        const host = mockHostRect(container);

        registeredDropHandler()({ payload: { type: "enter", paths: [], position: { x: 100, y: 100 } } });

        await waitFor(() => expect(host.hasAttribute("data-drag-over")).toBe(true));
      });

      it("clears the drag-over affordance on leave", async () => {
        const { container } = render(<ControlledNoteEditor rootId="01ROOT" path="note.md" />);
        await screen.findByText("Loaded");
        const host = mockHostRect(container);
        ...
        registeredDropHandler()({ payload: { type: "leave" } });

        await waitFor(() => expect(host.hasAttribute("data-drag-over")).toBe(false));
      });

      it("ignores a drag positioned outside the editor host", async () => {
        const { container } = render(<ControlledNoteEditor rootId="01ROOT" path="note.md" />);
        await screen.findByText("Loaded");
        const host = mockHostRect(container);

        registeredDropHandler()({ payload: { type: "over", paths: [], position: { x: 9999, y: 9999 } } });

        expect(host.hasAttribute("data-drag-over")).toBe(false);
      });

      it("imports a dropped file and inserts its reference, clearing the drag-over affordance", async () => {
        ...
        registeredDropHandler()({
          payload: { type: "drop", paths: ["/home/user/dropped.png"], position: { x: 100, y: 100 } },
        });

        await waitFor(() =>
          expect(invoke).toHaveBeenCalledWith("import_attachment", {
            rootId: "01ROOT",
            absolutePath: "/home/user/dropped.png",
          }),
        );
        await screen.findByText(/!\[image\]\(attachment:01DROP\)/);
        expect(host.hasAttribute("data-drag-over")).toBe(false);
      });

      it("imports each dropped path independently, naming the rejected file without dropping the rest of the batch", async () => {
        invoke.mockImplementation((command: string, args?: { absolutePath?: string }) => {
          if (command === "open_note") return Promise.resolve({ content: "Loaded\n", id: "01LOADED", is_conflicted: false });
          if (command === "import_attachment") {
            if (args?.absolutePath === "/home/user/bad.txt") {
              return Promise.reject(new Error("not a recognized image format (PNG, JPEG, GIF, or WebP)"));
            }
            return Promise.resolve("attachment:01GOOD");
          }
          return Promise.resolve(undefined);
        });
        ...
        const { container } = render(<ControlledNoteEditor rootId="01ROOT" path="note.md" />);
        await screen.findByText("Loaded");
        mockHostRect(container);

        registeredDropHandler()({
          payload: {
            type: "drop",
            paths: ["/home/user/bad.txt", "/home/user/good.png"],
            position: { x: 100, y: 100 },
          },
        });

        await screen.findByText(/!\[image\]\(attachment:01GOOD\)/);
        const alert = await screen.findByRole("alert");
        expect(alert.textContent).toContain("/home/user/bad.txt");
        expect(alert.textContent).toContain("not a recognized image format");
      });

      it("ignores a drop positioned outside the editor host", async () => {
        const { container } = render(<ControlledNoteEditor rootId="01ROOT" path="note.md" />);
        await screen.findByText("Loaded");
        mockHostRect(container);

        registeredDropHandler()({
          payload: { type: "drop", paths: ["/home/user/dropped.png"], position: { x: 9999, y: 9999 } },
        });
